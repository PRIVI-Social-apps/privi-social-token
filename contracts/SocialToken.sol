...
contract SocialToken is ERC20, Ownable, ReentrancyGuard, BNum {
    enum AMMType {LINEAR, QUADRATIC, EXPONENTIAL, SIGMOID}

    // constants
    uint256 constant eN = 271828; // mathmatical e
        ...
        returns (uint256)
    {
        ...
        if (ammType == AMMType.EXPONENTIAL) {
            (uint256 mutiPlierPowerResult, uint256 multiPlierPowerDivision) =
                bpow(eD, eN, targetSupply);
            multiplier = PRECISION * targetPrice * mutiPlierPowerResult;

            (uint256 upperPowerResult, uint256 upperPowerDivision) =
                bpow(eN, eD, upperBound);
            (uint256 lowerPowerResult, uint256 lowerPowerDivision) =
                bpow(eN, eD, upperBound);

            if (upperPowerResult > lowerPowerResult) {
                integral = upperPowerResult - lowerPowerResult;
            } else {
                integral = lowerPowerResult - upperPowerResult;
            }
    ...
    function priceToMint(uint256 amount) internal returns (uint256) {
        ...
        (effectiveSupply, ) = bsubSign(initialSupply, totalSupply());
